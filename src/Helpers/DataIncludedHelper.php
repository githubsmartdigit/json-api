 ...
 */
namespace Xooxx\JsonApi\Helpers;
...
class DataIncludedHelper
{
     ...
     */
    protected static function addToRelationshipsArray(array &$mappings, array &$data, array &$value, $type, array &$relationships, array &$attributes)
    {
        foreach ($value as $propertyName => $attribute) {
            if (PropertyHelper::isAttributeProperty($mappings, $propertyName, $type)) {
                ...
                if (\array_key_exists(Serializer::MAP_TYPE, $attribute) && count(array_values($attribute[Serializer::SCALAR_VALUE])) > 0 && \array_key_exists(Serializer::CLASS_IDENTIFIER_KEY, array_values($attribute[Serializer::SCALAR_VALUE])[0])) {
                    self::setResponseDataIncluded($mappings, $value, $data);
                    continue;
                }
                if (\array_key_exists(Serializer::CLASS_IDENTIFIER_KEY, $attribute)) {
                    self::setResponseDataIncluded($mappings, $value, $data);
                    $relationships[$propertyName] = \array_merge(DataLinksHelper::setResponseDataLinks($mappings, $attribute), [JsonApiTransformer::DATA_KEY => [$propertyName => PropertyHelper::setResponseDataTypeAndId($mappings, $attribute)]], $mappings[$type]->getRelationships());
                    continue;
                }
    ...
    protected static function addToIncludedArray(array &$mappings, array &$data, array &$attributes, array &$value)
    {
        if (\count($attributes) > 0) {
            ...
            if (self::hasIdKey($includedData)) {
                ...
                $relationshipData = [];
                self::addRelationshipsToIncludedResources($mappings, $relationshipData, $value, $value[Serializer::CLASS_IDENTIFIER_KEY]);
                if ($relationshipData) {
                    ...
                    $relationships = self::normalizeRelationshipData($arrayData);
                ...
                if ($existingIndex !== false) {
                    $data[JsonApiTransformer::INCLUDED_KEY][$existingIndex] = \array_filter(\array_merge($data[JsonApiTransformer::INCLUDED_KEY][$existingIndex], \array_filter($arrayData, self::filterEmptyArray())), self::filterEmptyArray());
                } else {
    ...
    {
        return \array_key_exists(JsonApiTransformer::ID_KEY, $includedData) && !empty($includedData[JsonApiTransformer::ID_KEY]);
    }
    /**
     ...
     * @return array
     */
    protected static function normalizeRelationshipData(array $arrayData)
    {
        $relationships = [];
    ...
    {
        return !empty($mappings[$parentType]) && count($mappings[$parentType]->getIncludedResources()) > 0 && false === in_array($includeValue[Serializer::CLASS_IDENTIFIER_KEY], $mappings[$parentType]->getIncludedResources(), true);
    }
}