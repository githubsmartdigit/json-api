...
class DataAttributesHelper
{
     ...
     * @var array
     */
    protected static $forbiddenAsFirstOrLastCharacter = ['-', '_', ' '];
    /**
    ...
    public static function setResponseDataAttributes(array &$mappings, array &$array)
    {
        ...
        $included = $mappings[$type]->getIncludedResources();


        foreach ($array as $propertyName => $value) {

            if(isset($value['_relation'])) {
                if (in_array($value['_relation'], $included)) {
                    $array[$propertyName] = $value = $value['_func'](false);
                } else {
                    $array[$propertyName] = $value = $value['_func'](true);
                }
            }
            ...
            if (\is_array($value) && !array_key_exists(Serializer::CLASS_IDENTIFIER_KEY, $value)) {
                ...
                if (!empty($value["@value"]) && false === self::hasClassIdentifierKey($value)) {
                    $attributes[$keyName] = $value;
                }
            }
        }
    ...
    {
        return \is_array($value) && \array_key_exists(Serializer::SCALAR_TYPE, $value) && \array_key_exists(Serializer::SCALAR_VALUE, $value);
    }
    /**
     * @param array $input
     *
