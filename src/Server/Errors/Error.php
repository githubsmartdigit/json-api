...
class Error implements JsonSerializable
{
    ...
    {
        $validStatus = [100, 101, 102, 200, 201, 202, 203, 204, 205, 206, 207, 208, 300, 301, 302, 303, 304, 305, 306, 307, 400, 401, 402, 403, 404, 405, 406, 407, 408, 409, 410, 411, 412, 413, 414, 415, 416, 417, 418, 422, 423, 424, 425, 426, 428, 429, 431, 500, 501, 502, 503, 504, 505, 506, 507, 508, 511];
        if (false === in_array($status, $validStatus, false)) {
            throw new \InvalidArgumentException(sprintf('Provided status does not match a valid HTTP Status Code.', $status));
        }
    ...
    {
        return array_filter(['id' => $this->id, 'links' => $this->links, 'status' => $this->status, 'code' => $this->code, 'title' => $this->title, 'detail' => $this->detail, 'source' => $this->source, 'meta' => $this->meta]);
    }
}